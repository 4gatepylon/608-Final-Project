...
uint32_t esp_now_timer; // main loop timer
uint32_t server_timer;  // loop timer for sending to the server
...
void loop()
{
  ...
  // Server requires its own timer to avoid crashing the Arduino
  // (Synchronize with Google API because it's easier that way)
  if (millis() - server_timer > DT_SERVER)
  {
  ...
  // esp now can be handled much more often which is why we do it on DT instead of DT_SERVER
  if (millis() - esp_now_timer > DT)
  {
    // send the position
  ...
  // Very short delay to enable reasonable speed switching
  delay(10);
}
