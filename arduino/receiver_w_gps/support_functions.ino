...
int wifi_object_builder(char *object_string, uint32_t os_len, uint8_t channel, int signal_strength, uint8_t *mac_address)
{
  ...
  char buffer[100];
  int added = 0;
  added += sprintf(buffer, "{\"macAddress\": \"%02x:%02x:%02x:%02x:%02x:%02x\",",
                   mac_address[0], mac_address[1], mac_address[2], mac_address[3], mac_address[4], mac_address[5]);
  added += sprintf(buffer + added, "\"signalStrength\": %d", signal_strength);
  added += sprintf(buffer + added, ",\"age\": %d", 0);
  added += sprintf(buffer + added, ",\"channel\": %d}", channel);
  if (os_len < strlen(buffer))
  {
    return 0;
  }
  strcat(object_string, buffer);
  return added;
}
